...
public class binarySearchTree<T> implements Comparable<Object>
{
	...
	private T  element;
	public BSTNode<T> root, tree, current, head, node;
	...
	// e.equals(element), otherwise returns false.
	{
		...
		if (tree==null) {System.out.println("tree======"+tree);
		return false;}
	...
	public void contains(T element)// Returns true if this BST contains an element e such that
	// e.equals(element), otherwise returns false.
	{
	...
	// Adds element to this BST. The tree retains its BST property.
	{
		...
		System.out.println("just left recAdd");
	...
	// Adds element to tree; tree retains its BST property.
	{
		...
		if (tree == null)// Addition place found
		{
			...
			System.out.println("recAdd tree =" + tree.getInfo());
		}
		else if (((Comparable<T>) element).compareTo(tree.getInfo()) <= 0){
			tree.setLeft(recAdd(element, tree.getLeft()));
			System.out.println("now in the left branch "+tree.getLeft());}
		else
	...
	public String toString(BSTNode<T> tree)
	{
		list ="";
		current = tree;
		while(current!=null)
		{
			...
			current=null; // current.getPointer();
		}
